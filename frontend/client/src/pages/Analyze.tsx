...
import { fileToDataUrl, getHistory, saveHistory } from "@/api/history";
import {
  AnalysisLoader,
  ResultView,
  UploadZone,
...
export default function Analyze() {
   ...
   *
   * The History page passes the ID of the saved entry.
   */
  useEffect(() => {
  ...
  const reset = () => {
    ...
    // Remove the historyId from the URL after resetting.
    window.history.replaceState({}, "", "/analyze");
  };
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        } else if (err.message === "invalid-image-data") {
          message =
    ...
    <div className="analyze-page">
      <div className="shell">
        <div className="analyze-header">
          <div>
            <span className="eyebrow eyebrow--rule">
              ANALYSIS WORKSPACE <span>LIVE</span>
            </span>

            <h1>Analyze an image</h1>

            <p>
              Inspect the likelihood of synthetic generation with visual
          ...
          <div className="analysis-state">
            <span
              className={`status-dot ${
                state === "loading" ? "status-dot--pulse" : ""
              }`}
            />

            {state === "loading" ? "Analyzing" : "Model online"}
          </div>
        </div>
          ...
          <div className="analysis-workspace">
                ...
                <div className="analysis-cta">
                  <div className="analysis-cta__hint">
                    {state === "empty" ? (
                      ...
                      <>
                        <AlertCircle size={17} />
                        Analysis stopped. Try again.
                      </>
                    ) : (
                      <>
                        <span className="status-dot" />
                        Image ready to inspect
                      </>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    onClick={runAnalysis}
                  >
                    {state === "error" ? "Try again" : "Analyze image"}
                    <ArrowRight size={17} />
                  </button>
                </div>
              )}
            ...
            <aside className="analysis-workspace__aside">
              <div className="aside-panel">
                ...
                <p>
                  SignalScope works best when the original image is clear and
                  has not been heavily recompressed.
                </p>

                <ul>
                  <li>Accepted: JPG, PNG, WEBP, BMP</li>
                  <li>Maximum file size: 15 MB</li>
                  <li>
                    Images are not stored on the analysis server. If you use
                    History, a local thumbnail is kept in your browser.
                  </li>
                </ul>
              </div>
              ...
              <div className="aside-panel aside-panel--technical">
                ...
                <div className="model-status-row">
                  <span>SignalScope classifier</span>
        ...
        <div className="workspace-footnote">
          <span className="mono">03 / NOTE</span>

          <p>
            SignalScope provides a likelihood assessment, not definitive proof
            of image origin. Performance can vary across image domains and
            transformations.
          </p>
